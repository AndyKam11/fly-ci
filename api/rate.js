...
// Vercel serverless function: logs every judged post to Supabase and returns the running count + percentile.
...
async function count(filter = '') {
  const r = await fetch(`${T()}?select=id${filter}`, { headers: { ...H(), Prefer: 'count=exact', Range: '0-0' } });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const fly_score = Number(b.fly_score) || 0;
    const token = randomBytes(32).toString('hex');
    const row = {
      is_public: false, publish_token_hash: createHash('sha256').update(token).digest('hex'),
      post, bait_score: Number(b.bait_score) || 0, bait: Array.isArray(b.bait) ? b.bait.slice(0, 20) : [],
      rate: Number(b.rate) || 0, run: Number(b.run) || 0, mn9: Number(b.mn9) || 0, fly_score,
    ...
    const r = await fetch(T(), { method: 'POST', headers: { ...H(), Prefer: 'return=representation' }, body: JSON.stringify(row) });
    if (!r.ok) return res.status(502).json({ ok: false });
    const [saved] = await r.json();
    const [total, below] = await Promise.all([count(), count(`&fly_score=lt.${fly_score}`)]);
    return res.status(200).json({ ok: true, id: saved.id, token, count: total, percentile: total > 1 ? Math.round(below / (total - 1) * 100) : null });
  } catch (e) {
